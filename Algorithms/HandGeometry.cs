...
namespace Algorithms
{
    public class HandGeometry : IHandGeometry
    {
        ...
        public List<Point[]> CalculateBaseLines(Bitmap img)
        {
            ...
            lstHandGeometryFeatures.Add(Geometry.EuclideanDistance(new Point(pStart.X, pStart.Y), ValleyPointsOrdered[_INDEX_FINGER_]));
            this.lstBaseLines.Add(new Point[2] { new Point(pStart.X, pStart.Y), ValleyPointsOrdered[_INDEX_FINGER_] });
        ...
        public Point[] GetFingerHeight(int fingerIndex, Bitmap img)
        {
            ...
            {
                case _INDEX_FINGER_:
                    iFingerHeight = this.CalculateFingerHeight(lstBaseLines[0], this.EdgePointsOrdered[_INDEX_FINGER_]);
                    break;

                case _MIDDLE_FINGER_:
                    iFingerHeight = this.CalculateFingerHeight(lstBaseLines[1], this.EdgePointsOrdered[_MIDDLE_FINGER_]);
                    break;

                case _RING_FINGER_:
                    iFingerHeight = this.CalculateFingerHeight(lstBaseLines[2], this.EdgePointsOrdered[_RING_FINGER_]);
                    break;

                case _PINKY_FINGER_:
                    iFingerHeight = this.CalculateFingerHeight(lstBaseLines[3], this.EdgePointsOrdered[_PINKY_FINGER_]);              
                    break;
            }

            return iFingerHeight;
        }
        ...
        public List<Point[]> CalculateIndexFingerWidths(Bitmap img)
        {
            ...
            Gap gap = new Gap(30,60,10, new Point(fbld.MiddleX, fbld.MiddleY));
            lstIndexFingerWidths = this.CalculateFingerWidths(gap, fbld.Inclination, img);
        ...
        public List<Point[]> CalculateMiddleFingerWidths(Bitmap img)
        {
            ...
            Gap gap = new Gap(50, 80, 10, new Point(fbld.MiddleX, fbld.MiddleY));
            lstMiddleFingerWidths = this.CalculateFingerWidths(gap, fbld.Inclination, img);
        ...
        public List<Point[]> CalculateRingFingerWidths(Bitmap img)
        {
            ...
            Gap gap = new Gap(50, 80, 10, new Point(fbld.MiddleX, fbld.MiddleY));
            lstRingFingerWidths = this.CalculateFingerWidths(gap, fbld.Inclination, img);
        ...
        public List<Point[]> CalculateDistancesFromCentroidToFingerTips()
        {
         ...
         /*if (type == "train")
         {
             UserData.Instance.AddTranning(fileLabel, lstHandGeometryFeatures);

             if (UserData.Instance.UserAttributesTrainning[fileLabel].Count > 1)
                 UserData.Instance.NormalizeData(fileLabel, type);
         }
         else
         {
             UserData.Instance.AddTesting(fileLabel, lstHandGeometryFeatures);
             UserData.Instance.NormalizeData(fileLabel, type);
         }*/
        ...
        private FingerBaseLineData CalculateFingerBaseLength(int fingerIndex, Bitmap img)
        {
            ...
            {
                Point pFirstValley = AdjusFirstValleyPoint(img);
                pCurrent = pFirstValley;
            }
            else
            {
            ...
            int iInclination = Geometry.AngularCoefficient(pCurrent.Y, this.ValleyPointsOrdered[1].Y);
        ...
        private List<Point[]> CalculateFingerWidths(Gap gap, int inclination, Bitmap img)
        {
            ...
            for (int iGap = gap.Start; iGap < gap.Limit; iGap += gap.Step)
            {
                ...
                while (img.GetPixel(iMiddleX, iMiddleY - iGap).Name == "ffffffff")
                {
                    iMiddleX--;
                }

                startPoint.X = iMiddleX;
                startPoint.Y = iMiddleY - iGap + inclination;

                int iMiddleXEnd = iMiddleX;
                while (img.GetPixel(iMiddleXEnd, iMiddleY - iGap).Name == "ffffffff")
                {
                    iMiddleXEnd++;
                }
        ...
        private void DetectHandValleys()
        {
            ...
            //Dictionary<Double, Point> shortest = new Dictionary<double, Point>();
